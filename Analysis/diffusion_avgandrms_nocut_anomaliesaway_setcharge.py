...
endlocation_in = '/Diffusion_bead_near_grid/Spacing'+str(spacing)+'/damp%i_diffseedLgv/Brush/Sigma_bead_' % damp + str(psigma) + '/Charge'+str(charge)+'/'
endlocation    = endlocation_in +'Nocut/'
filestext      = 'config'+str(confignrs[0])+'to'+str(confignrs[-1])
# Text files
outfilename    = endlocation+'diffusion'+filestext+'_better_rms_Nestimates%i_charge' % Nrms+str(charge)+'.txt' 
metaname       = endlocation+'diffusion_metadata'+filestext+'_better_rms_Nestimates%i_charge' % Nrms+str(charge)+'.txt'

# Plots
plotname       = endlocation+filestext+'_nocut_better_rms_Nestimates%i_charge' % Nrms+str(charge)+'_compare.png'


## Getting interval to perform the fit:
rangefilename = endlocation+'indices_for_fit_Nestimates%i_charge' % Nrms+str(charge)+'.txt'
...
plt.title(r'Averaged RMSD in brush, d = %.2f nm, $\sigma_b=%.2f$, $q$ = %.2f' % (spacing,psigma,charge))
plt.tight_layout()
