...
for spacing in spacings:
    ...
    for confignr in confignrs:
        filename = '/Diffusion_bead_near_grid/Spacing%s/damp%i_diffseedLgv/Brush/Sigma_bead_%s/log-files/log.confignr%i_printevery10' % (str(spacing),dampfac,str(sigmabead),confignr)
        try:
        ...
        while i<totlines:
            ...
            if startread==True:
                if words[0]!='Loop' and words[0]!='WARNING:':
                    ...
                    break
            if len(words)>0:
                if words[0]=='Step' and words[1]=='c_peprint':
                    startread=True
            i+=1
        infile.close()
