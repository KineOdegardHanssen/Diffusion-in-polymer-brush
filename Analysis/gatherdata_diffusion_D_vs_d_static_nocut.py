...
psigma   = 1 
spacings = [1,1.25,1.5,2,2.5,3,3.5,4,4.5,5,6,7,8,10,25,50,75,100]
damp     = 10
N        = len(spacings)
...
for i in range(N):
    ...
    startindex_R   = int(mlines[2].split()[1])
    endindex_R     = int(mlines[3].split()[1])
    startindex_ort = int(mlines[4].split()[1])
    endindex_ort   = int(mlines[5].split()[1])
    startindex_par = int(mlines[6].split()[1])
    endindex_par   = int(mlines[7].split()[1])
    metafile.close()
...
plt.title('Diffusion constant $D$, d = %i nm, static %s, no cut' % (spacing, systemtype))
plt.tight_layout()
