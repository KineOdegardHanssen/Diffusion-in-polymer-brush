...
endlocation_in = '/Diffusion_bead_near_grid/Spacing'+str(spacing)+'/damp%i_diffseedLgv/Pure_bulk/Sigma_bead_' % damp+str(psigma) + '/'
endlocation    = endlocation_in
filestext      = '_seed'+str(confignrs[0])+'to'+str(confignrs[-1])
# Text files
outfilename    = endlocation+'diffusion'+filestext+'_better_rms_Nestimates%i_pmass1.txt' % Nrms
metaname       = endlocation+'diffusion_metadata'+filestext+'_better_rms_Nestimates%i_pmass1.txt' % Nrms

# Plots
plotname       = endlocation+filestext+'_better_rms_Nestimates%i_compare_pmass1.png' % Nrms
...
for j in range(Nrms):
    ...
    for k in range(N_per_rms):
        ...
        if long==True:
            infilename_free = endlocation_in+'long/'+'seed'+str(confignr)+'_pmass1_long.lammpstrj'
        else:
            infilename_free = endlocation_in+'seed'+str(confignr)+'_pmass1.lammpstrj'
        ...
        try:
            ...
            try:
                infilename_free = endlocation_in+'long/'+'seed'+str(confignr)+'_pmass1.lammpstrj'
                infile_free = open(infilename_free, "r")
            except:
        ...
        lines = infile_free.readlines() # This may take some time
        ...
        while i<totlines:
            ...
            if (words[0]=='ITEM:'):
                ...
                # Find properties
                # Order:  id  type ux  uy  uz  vx  vy   vz
                #         [0] [1] [2] [3] [4] [5] [6]  [7]
...
#'''
plt.figure(figsize=(6,5))
plt.plot(tot_averageR2s_SI, label=r'$<dR^2>$')
plt.plot(tot_averagedz2s_SI, label=r'$<dz^2>$')
plt.plot(tot_averagedparallel2_SI, label=r'$<dx^2+dy^2>$')
plt.plot(fit_DR_SI, '--', label=r'$<dR^2>$, fit')
plt.plot(fit_Dz_SI, '--', label=r'$<dz^2>$, fit')
plt.plot(fit_Dpar_SI, '--', label=r'$<dx^2+dy^2>$, fit')
