...
psigma   = 1  
charge   = -0.01
spacings = [2,2.5,3,3.5,4,4.5,5,6,7,8,10] 
damp     = 10
N        = len(spacings)
Nrms     = 10 
...
endlocation_out = '/Diffusion_bead_near_grid/D_vs_d/'+parentfolder+'Sigma_bead_' +str(psigma) + '/Nocut/'+ 'Charge' + str(charge)+'/'
...
outfile.write('d   D_R2   sigmaD_R2; D_z2  sigmaD_z2; D_par2 sigmaD_par2\n')
...
for i in range(N):
    spacing = spacings[i]
    endlocation_in   = '/Diffusion_bead_near_grid/Spacing'+str(spacing)+'/damp%i_diffseedLgv/' % damp +parentfolder+ 'Sigma_bead_' +str(psigma) +'/Charge'+str(charge)+'/Nocut/'
...
plt.ylabel(r'Diffusion constant $D$',fontsize=14)
