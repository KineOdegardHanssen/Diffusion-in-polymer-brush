...
endlocation_cut     = '/Diffusion_bead_near_grid/Spacing'+str(spacing)+'/damp%i_diffseedLgv/Brush/Sigma_bead_' % damp+str(psigma) + '/Charge'+str(charge)+'/'
...
plt.title(r'$<R^2>$ in brush, d = %.2f nm, $\sigma_b=%.2f$, $q$ = %.2f' % (spacing,psigma,charge))
...
plt.title(r'$<dz^2>$ in brush, d = %.2f nm, $\sigma_b=%.2f$, $q$ = %.2f' % (spacing,psigma,charge))
ax.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
box = ax.get_position()
ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
...
plt.title(r'RMSD in brush, d = %.2f nm, $\sigma_b=%.2f$, $q$ = %.2f' % (spacing,psigma,charge))
ax.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
box = ax.get_position()
ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
